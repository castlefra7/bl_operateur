...
public class Customer extends Person {
    ...
    private Date lastCreditOpDate(Connection conn) throws SQLException {
        String req = String.format("select created_at from mg.buyed_credits where customer_id = %d", getId());
         return FctGen.getDate(req, "created_at", conn);
    }
    
    private Date lastOperationDate(Date date, Connection conn) throws SQLException {
        String req = String.format("select max(((select created_at from mg.deposits where customer_id = %d  order by created_at desc limit 1) union (select created_at from mg.withdraws where customer_id = %d order by created_at desc limit 1)));", getId(), getId());
